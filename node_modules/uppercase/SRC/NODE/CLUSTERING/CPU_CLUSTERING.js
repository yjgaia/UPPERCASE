		...
		run: (work) => {
			...
			if (CPU_COUNT <= 1) {
				...
				if (Cluster.isMaster === true) {
					...
					let createWorker = (workerId) => {

						let newWorker = Cluster.fork();

						childWorkerIds[newWorker.id] = workerId;

						// receive params from new worker.
						newWorker.on('message', (params) => {

							if (params.methodName === '__REQUEST_WORKER_ID') {

								newWorker.send({
									methodName: '__SET_WORKER_ID',
									data: workerId
								});
							}

							// send.
							else if (params.workerId !== undefined) {
					...
					Cluster.on('exit', (worker, code, signal) => {
						...
						const workerId = childWorkerIds[worker.id];
				...
				// when worker
					...
					process.send({
						methodName: '__REQUEST_WORKER_ID'
					});
				}
			}
		}
